...
class MetricOverrideReporter(wrappedReporter: MetricReporter,
    ...
    tags
      ...
      .foreach(tag => {
        if (!mapping.tagsToDelete.contains(tag.key)) {
          remappedTags.add(mapping.tagsToRename.getOrElse(tag.key, tag.key),
                           Tag.unwrapValue(tag).toString)
        }
      })
  ...
  private def updateDistribution[T <: Metric.Settings, U](
      metric: MetricSnapshot[T, U]): MetricSnapshot[T, U] = {
    metricsMap
      .get(metric.name)
      .map(mapping => {

        val mappedInstruments =
          if (mapping.tagsToRename.isEmpty && mapping.tagsToDelete.isEmpty)
            metric.instruments
          else {
         ...
         MetricMapping(
           ...
           if (config.hasPath("rename-tags"))
             config
               ...
               .asScala
               .toMap
               .map { case (tagName, value) => (tagName, value.toString) } else
             Map.empty
         ))
    }
  }
