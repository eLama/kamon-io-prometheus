...
class PrometheusReporter(configPath: String) extends MetricReporter {
  ...
  private val _snapshotAccumulator =
    PeriodSnapshot.accumulator(Duration.ofDays(365 * 5),
                               Duration.ZERO,
  ...
  override def reportPeriodSnapshot(snapshot: PeriodSnapshot): Unit = {
    ...
    val reporterConfiguration = readSettings(
      Kamon.config().getConfig(configPath))
  ...
  object Settings {
    ...
    def readSettings(prometheusConfig: Config): PrometheusReporter.Settings = {
      PrometheusReporter.Settings(
        ...
        defaultBuckets = prometheusConfig
          .getDoubleList("buckets.default-buckets")
          .asScala
          .toSeq,
        timeBuckets =
          prometheusConfig.getDoubleList("buckets.time-buckets").asScala.toSeq,
        informationBuckets = prometheusConfig
          .getDoubleList("buckets.information-buckets")
          .asScala
          .toSeq,
        customBuckets =
    ...
    def environmentTags(
        reporterConfiguration: PrometheusReporter.Settings): TagSet =
      ...
      customBuckets.topLevelKeys
        .map(
          k =>
            (k,
             customBuckets
               .getDoubleList(ConfigUtil.quoteString(k))
               .asScala
               .toSeq))
        .toMap
  }
