...
class ScrapeDataBuilder(prometheusConfig: PrometheusReporter.Configuration,
  ...
  def appendCounters(counters: Seq[MetricValue]): ScrapeDataBuilder = {
    counters
      .groupBy(_.name)
      .foreach(appendValueMetric("counter", alwaysIncreasing = true))
    this
  }
  ...
  def appendGauges(gauges: Seq[MetricValue]): ScrapeDataBuilder = {
    gauges
      .groupBy(_.name)
      .foreach(appendValueMetric("gauge", alwaysIncreasing = false))
    this
  }
  ...
  private def resolveBucketConfiguration(
      metric: MetricDistribution): Seq[java.lang.Double] =
  ...
  private def appendPercentiles(name: String,
    ...
    var currentDistributionPercentileValue =
          ...
          if (currentDistributionPercentileValue <= configuredBucket * 100) {
            inBucketValue =
              Math.max(inBucketValue,
                       scale(currentDistributionPercentile.value, metric.unit))
          } else
  ...
  private def appendHistogramBuckets(name: String,
    ...
    var currentDistributionBucketValue =
      scale(currentDistributionBucket.value, metric.unit)
  ...
  private def scale(value: Long, unit: MeasurementUnit): Double =
    unit.dimension match {
