...
class ScrapeDataBuilderSpec extends WordSpec with Matchers {
  ...
  "the scrape data builder formatter" should {
    ...
    "append units to the metric names when reporting values in the information dimension" in {
      val counterOne =
        MetricValue("counter-one", Map.empty, information.bytes, 10)
    ...
    "normalize tag names" in {
      val counterOne = MetricValue("app:counter-one",
                                   Map("tag.with.dots" -> "value"),
                                   time.seconds,
                                   10)
      val gaugeOne = MetricValue("gauge-one",
                                 Map("tag-with-dashes" -> "value"),
                                 time.seconds,
                                 20)
    ...
    "append counters and group them together by metric name" in {
      ...
      val counterOneWithTag =
        MetricValue("counter-one", Map("t" -> "v"), none, 30)

      builder()
        .appendCounters(Seq(counterOne, counterTwo, counterOneWithTag))
        .build() should include {
        """
    ...
    "append gauges and group them together by metric name" in {
      ...
      val gaugeTwoWithTags =
        MetricValue("gauge-two", Map("t" -> "v", "t2" -> "v2"), none, 30)

      builder()
        .appendGauges(Seq(gaugeOne, gaugeTwo, gaugeTwoWithTags))
        .build() should include {
        """
    ...
    "read custom bucket configurations" in {
      val config = PrometheusReporter.Configuration.readConfiguration(
        ConfigFactory
          .parseString(
            """
    ...
    "override histogram buckets with custom configuration" in {
      val customBucketsHistogram =
        constantDistribution("histogram.custom-buckets", Map.empty, none, 1, 10)
      ...
      builder(
        ...
        """
          ...
          |histogram_custom_buckets_percentile{quantile="0.75"} 7.0
          |histogram_custom_buckets_percentile{quantile="0.9"} 9.0
          |histogram_custom_buckets_percentile{quantile="0.95"} 9.0
    ...
    "append histograms grouped together by metric name and with all their derived time series " in {
      ...
      val histogramTwo =
        constantDistribution("histogram-two", Map.empty, none, 1, 20)
      val histogramThree =
        constantDistribution("histogram-three", Map.empty, none, 5, 10)
      val histogramWithZero =
        constantDistribution("histogram-with-zero", Map.empty, none, 0, 10)
      ...
      builder(buckets = Seq(15D))
        ...
        """
          ...
          |histogram_one_percentile{quantile="0.75"} 7.0
          |histogram_one_percentile{quantile="0.9"} 9.0
          |histogram_one_percentile{quantile="0.95"} 9.0
      ...
      builder(buckets = Seq(5D, 10D, 15D, 20D))
        ...
        """
          ...
          |histogram_two_percentile{quantile="0.75"} 15.0
          |histogram_two_percentile{quantile="0.9"} 18.0
          |histogram_two_percentile{quantile="0.95"} 19.0
      ...
      builder(buckets = Seq(3D))
        ...
        """
          ...
          |histogram_three_percentile{quantile="0.75"} 8.0
          |histogram_three_percentile{quantile="0.9"} 9.0
          |histogram_three_percentile{quantile="0.95"} 9.0
      ...
      builder(buckets = Seq(0.005D, 0.05D, 0.5D, 1D, 2D, 2.1D, 2.2D, 2.3D, 10D))
        ...
        """
          ...
          |histogram_with_zero_percentile{quantile="0.75"} 7.0
          |histogram_with_zero_percentile{quantile="0.9"} 8.0
          |histogram_with_zero_percentile{quantile="0.95"} 9.0
    ...
    "include global custom tags from the Kamon.environment.tags" in {
      val counterOne = MetricValue("counter-one",
                                   Map("tag.with.dots" -> "value"),
                                   time.seconds,
                                   10)
      val gaugeOne = MetricValue("gauge-one", Map.empty, time.seconds, 20)
    ...
    "let environment tags have precedence over custom tags" in {
      val metric = MetricValue("some-metric",
                               Map("custom.tag" -> "custom-value"),
                               time.seconds,
                               10)
  ...
  private def builder(
      ...
      customBuckets: Map[String, Seq[java.lang.Double]] = Map(
        "histogram.custom-buckets" -> Seq(1D, 3D)),
      percentiles: Seq[java.lang.Double] =
        Seq(0.10d, 0.25d, 0.50d, 0.75d, 0.90d, 0.95d, 0.99d, 0.999d),
      environmentTags: Map[String, String] = Map.empty) = new ScrapeDataBuilder(
    PrometheusReporter.Configuration(false,
                                     "localhost",
                                     1,
                                     buckets,
                                     buckets,
  ...
  private def constantDistribution(name: String,
                                   tags: Map[String, String],
                                   unit: MeasurementUnit,
                                   lower: Int,
                                   upper: Int): MetricDistribution = {
