...
class ScrapeDataBuilderSpec extends WordSpec with Matchers {
  ...
  "the scrape data builder formatter" should {
    "append units to the metric names when reporting values in the time dimension" in {
      val counterOne = MetricSnapshotBuilder.counter("counter-one",
                                                     "",
                                                     TagSet.Empty,
                                                     time.seconds,
                                                     10)
      val gaugeOne = MetricSnapshotBuilder.gauge("gauge-one",
                                                 "",
                                                 TagSet.Empty,
                                                 time.seconds,
                                                 20)
    ...
    "normalize tag names" in {
      val counterOne =
        MetricSnapshotBuilder.counter("app:counter-one",
                                      "",
                                      TagSet.of("tag.with.dots", "value"),
                                      time.seconds,
                                      10)
      val gaugeOne =
        MetricSnapshotBuilder.gauge("gauge-one",
                                    "",
                                    TagSet.of("tag-with-dashes", "value"),
                                    time.seconds,
                                    20)
    ...
    "read custom bucket configurations" in {
      val config = PrometheusReporter.Settings.readSettings(
        ConfigFactory
          .parseString(
            """
    ...
    "override histogram buckets with custom configuration" in {
      val customBucketsHistogram =
        constantDistribution("histogram.custom-buckets", none, 1, 10)
    ...
    "append histograms grouped together by metric name and with all their derived time series " in {
      ...
      val histogramWithZero =
        constantDistribution("histogram-with-zero", none, 0, 10)
    ...
    "include global custom tags from the Kamon.environment.tags" in {
      val counterOne =
        MetricSnapshotBuilder.counter("counter-one",
                                      "",
                                      TagSet.of("tag.with.dots", "value"),
                                      time.seconds,
                                      10)
    ...
    "let environment tags have precedence over custom tags" in {
      val counter =
        MetricSnapshotBuilder.counter("some-metric",
                                      "",
                                      TagSet.of("custom.tag", "custom-value"),
                                      time.seconds,
                                      10)
      val gauge =
        MetricSnapshotBuilder.gauge("some-metric",
                                    "",
                                    TagSet.of("custom.tag", "custom-value"),
                                    time.seconds,
                                    10D)
  ...
  private def builder(
      ...
      environmentTags: TagSet = TagSet.Empty) = new ScrapeDataBuilder(
    PrometheusReporter.Settings(false,
                                "localhost",
                                1,
                                buckets,
                                buckets,
  ...
  private def constantDistribution(name: String,
                                   ...
                                   lower: Long,
                                   upper: Long): MetricSnapshot.Distributions =
    MetricSnapshotBuilder.histogram(name, "", TagSet.Empty)(
      (lower to upper): _*)
}
